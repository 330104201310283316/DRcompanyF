function submit() {
		...
		var path =document.getElementById("bq").html;
		var body = {
			"htmlTitle": Title,
			"htmlExplain": Explain,
			"htmlContent": html,
		...
		$(".AttachedPath").change(function() {
			...
			if (explorer.indexOf('MSIE') >= 0) {
				...
				if (!/\.(pdf)$/.test(imgSrc)) {
					...
					var url = URL.createObjectURL(file);
					var formData = new FormData();
					formData.append('file', document.getElementById('AttachedPath').files[0]);
					$.ajax({
						url: "http://192.168.3.49:8081/api/dr/UpLoadWord/UploadFileAsync",
						type: "post",
						data: formData,
						...
						processData: false,
						success: function(data) {
							document.getElementById("bq").html =data.data["url"];
							$(document.getElementsByTagName('iframe')[0].contentWindow.document.body).append(
								"<iframe   src='" + data.data["url"] + "'width='100%' height='100%'> ");
						},
						error: function(data) {
							alert("上传失败")
						}
